...
interrupt_entry:
    ...
    push fs
    push gs
    pusha   ; 入栈顺序：eax → ecx → edx → ebx → esp（入栈前的旧值） → ebp → esi → edi
...
interrupt_exit:
    ...
    ; 恢复下文寄存器信息
    popa    ; 出栈顺序：edi → esi → ebp → esp（弹出但不赋值） → ebx → edx → ecx → eax
    pop gs
    pop fs
    pop es
    pop ds
...
INTERRUPT_HANDLER 0x23, 0; 串口2 中断
INTERRUPT_HANDLER 0x24, 0; 串口1 中断
INTERRUPT_HANDLER 0x25, 0
INTERRUPT_HANDLER 0x26, 0
INTERRUPT_HANDLER 0x27, 0
INTERRUPT_HANDLER 0x28, 0; 实时时钟中断
