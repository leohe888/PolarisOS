...
{
    u8 ei_magic[4];    // ELF 魔数， 0x7F, 'E', 'L', 'F'
    u8 ei_class;       // 文件种类，1 表示 32 位，2 表示 64 位
    u8 ei_data;        // 大小端，1 表示小端，2 表示大端
...
typedef struct Elf32_Ehdr
{
    ...
    Elf32_Half e_phentsize; // program header entry size，程序头表条目大小
    Elf32_Half e_phnum;     // program header number，程序头数量
    Elf32_Half e_shentsize; // section header entry size，节头表条目大小
    Elf32_Half e_shnum;     // section header number，节头数量
...
static void load_segment(inode_t *inode, Elf32_Phdr *phdr)
{
    ...
    if (phdr->p_filesz < phdr->p_memsz) // 如果段在文件中不足，则填充 0；这是 .bss 段的特殊情况，
                                        // 因为 .bss 不存实际数据，只需要在内存里开辟空间并清零，所以文件里不占空间，p_memsz > p_filesz 的差值，就是 .bss 的大小。
    {
...
static u32 copy_argv_envp(char *filename, char *argv[], char *envp[])
{
    ...
    int len = 0;
    // 将用户态的 envp 拷贝到临时的内核栈
    for (int i = envc - 1; i >= 0; i--) // 从后往前拷贝
    {
        // 计算长度（包括结尾的 '\0'）
        len = strlen(envp[i]) + 1;
        // 得到拷贝地址
