...
static list_t free_list;              // 空闲缓冲区链表，被释放的缓冲区会放入此链表
static list_t wait_list;              // 等待进程链表
static list_t hash_table[HASH_COUNT]; // 缓冲区哈希表
...
static buffer_t *get_from_hash_table(dev_t dev, idx_t block)
{
    ...
    // 如果 bf 在空闲缓冲区链表中，则移除，一种优化，brelse 后并不是真的释放掉，而是放入空闲链表
...
void brelse(buffer_t *bf)
{
    ...
    if (!bf->count)
    {
        if (bf->rnode.next) // 如果已经加入了某一个链表，则在此链表中移除
        {
            list_remove(&bf->rnode);
        }
    ...
    {
        task_t *task = element_entry(task_t, node, list_pop_back(&wait_list));
        task_unblock(task);
    }
}
