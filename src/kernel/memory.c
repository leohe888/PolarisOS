...
static u32 memory_base = 0; // 可用物理内存基地址，应该等于 1M
static u32 memory_size = 0; // 可用物理内存大小
static u32 total_pages = 0; // 所有物理内存页数
static u32 free_pages = 0;  // 空闲物理内存页数
...
void mapping_init()
{
    ...
    // 将最后一个页表指向页目录自己，方便修改，但是会浪费 4MB 的内存
...
{
    assert(count > 0);
    i32 index = bitmap_scan(map, count);
...
void unlink_page(u32 vaddr)
{
    ...
    page_entry_t *pte = get_pte(vaddr, true);   // 如果页表没有连接物理页面，按理说应该要释放它，这里没有实现
    page_entry_t *entry = &pte[TIDX(vaddr)];
...
page_entry_t *copy_pde(void)
{
    ...
    for (size_t didx = 2; didx < 1023; didx++)  // 前两个页目录项指向是内核页表，最后一个页目录项指向页目录自己，不需要拷贝
    {
        ...
        u32 paddr = copy_page(pte); // 此处，页表没有使用写时复制，可以优化
    ...
    u32 vector0, u32 error, u32 eip, u32 cs, u32 eflags)
{
    ...
    if (code->present)
    {
        ...
        page_entry_t *pte = get_pte(vaddr, false);  // 获取页表
        page_entry_t *entry = &pte[TIDX(vaddr)];    // 获取页表项
