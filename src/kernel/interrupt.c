...
void pic_init(void)
{
    ...
    outb(PIC_M_CTRL, 0b00010001); // ICW1 —— 边沿触发, 级联 8259, 需要 ICW4.
    outb(PIC_M_DATA, 0x20);       // ICW2 —— 起始中断向量号 0x20
    outb(PIC_M_DATA, 0b00000100); // ICW3 —— IR2 接从片.
    outb(PIC_M_DATA, 0b00000001); // ICW4 —— 8086 模式, 手动 EOI

    outb(PIC_S_CTRL, 0b00010001); // ICW1 —— 边沿触发, 级联 8259, 需要 ICW4.
...
void idt_init(void)
{
    ...
    for (int i = 0; i < ENTRY_SIZE; i++)
    {
        ...
        gate->offset0 = (u32)handler & 0xffff;          // 中断处理函数偏移地址第 0 ~ 15 位
        gate->offset1 = ((u32)handler >> 16) & 0xffff;  // 中断处理函数偏移地址第 16 ~ 31 位
