...
void arena_init(void)
{
    ...
    for (size_t i = 0; i < DESC_COUNT; i++)
    {
        ...
        desc->block_size = block_size;  // block_size: 16 32 64 128 256 512 1024
...
static block_t *get_block(arena_t *arena, u32 idx)
{
    assert(idx < arena->desc->total_block);
    void *addr = (void *)(arena + 1);
    u32 gap = idx * arena->desc->block_size;
    return (block_t *)((u32)addr + gap);
}

static arena_t *get_arena(block_t *block)
{
...
void *kmalloc(size_t size)
{
    ...
    if (size > 1024)
    {
        u32 total_size = size + sizeof(arena_t);
        u32 page_count = div_round_up(total_size, PAGE_SIZE);
    ...
    if (list_empty(&desc->free_list))
    {
        ...
        {
            block = get_block(arena, i);
            assert(!list_search(&desc->free_list, block));
            list_push_front(&desc->free_list, block);
            assert(list_search(&desc->free_list, block));
        }
    }

    block = list_pop_front(&desc->free_list);
    arena = get_arena(block);
...
void kfree(void *ptr)
{
    ...
    list_push_front(&arena->desc->free_list, block);
    arena->count++;
